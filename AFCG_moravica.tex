...
\newcommand{\bbbf}{{}} %%% Bibliography bold face - espacio vacio - temporarily
...
\title[On G - transitive version of perfectly meager sets]{
...
\keywords{Perfectly meager sets, strongly meager sets, $\afcp$ sets.\\
\indent {\textit{Full paper}. Received (...),
revised 14 February 2020, accepted (...), 
available online (...)}}

\begin{abstract}
  We study the $G-$ invariant version of perfectly meager 
sets (a generalization of the notion of
$\afcp$ sets). We find the necessary and sufficient conditions for the inclusion
$\afcg \seq \calI$. In particular, we partially characterize for which groups
$G$ of automorphisms of the Cantor space every $\afcg$ set is Lebesgue null.
\end{abstract}
...
basic concept of arithmetic of cardinal numbers.
In particular, we need the notion of cofinality;
recall that an uncountable cardinal number $\kappa$ is called 
{\it regular} iff $\cf(\kappa) = \kappa$.
...
\end{definition}
  ...
  Notice that if $\calT$ is a $\sigma$-ideal (which contains singletons)
and $\kappa = \cof(\calI) = \cov(\calI)$ then
there exists a $\kappa$ - Sierpi\'nski set $X$ with
respect to $\calI$.

  Recall the classical definition of perfectly meager sets 
...
%\]
   ...
   Notice that the notion $\afcp$ is a strengthening of the 
 classical perfectly meager sets.

  The following notion was first defined by Karel Prikry:
(see \cite{C}, introduction):
\begin{definition}
...
\end{definition}
  ...
  Notice that K.Prikry conjectured that the collection of strongly meager sets form
a $\sigma$-ideal but it turned out that it is consistent 
that strongly meager sets are exactly the countable sets (see \cite{C}) and
that it is consistent that even the sum of two strongly meager 
sets need not be strongly meager set (see \cite{BS}). 

  It is known (see for example \cite{NSW} and \cite{NS2}),
that $\afcp \seq \afc$ and every strongly meager set is an $\afcp$ set.
  
  It is also known (see \cite{P}) that every Sierpi\'nski set is
strongly meager.

  We can summarize all these inclusions in Fig. \ref{figure0}  

\begin{figure}[h]
\setlength{\unitlength}{1mm}
\begin{picture}(140,50)(5,0)
\put(5,40){Sierpi\'nski set}
\put(50,40){SFC}
\put(80,40){$\afcp$}
\put(110,40){$\afc$}
\put(30,41){\vector(1,0){15}}
\put(62,41){\vector(1,0){15}}
\put(94,41){\vector(1,0){15}}
\end{picture}
\label{figure0}
...
%%%%TODO: wyjasnic dlaczego ref jest tutaj jako 2, przeciez nie ma nigdzie figure 1!
%%%% co sie dzieje?
...
\begin{question}
Can we prove the equivalence from Theorem \ref{em<=>afcg->i}
...
$(Em)$ in terms of the coefficient $Em(\calF, G)$.
...
Let X be a strongly meager set and let $Q$ be an arbitrary perfect set.
Since $\neg (Em)_{\neglig}$ we obtain that there exists a perfect
set $P$ such that $\mu (P) > 0$ and $\forall_{g \in G} g(Q) \cap P \in \mgr(g(Q))$.
Let $C \seq \cantor $ be a countable set such that
...
Then $X$ is strongly meager (\cite{P}).
...
\bibitem{BS} T. Bartoszy\'nski, S. Shelah, \textit{
Strongly meager sets do not form an ideal}, 
{\bbbf Journal of Mathematical Logic}
Vol. 01, No. 01, pp. 1-34 (2001).

\bibitem{C} T.J. Carlson
 \textit{Strong measure zero and strongly meager sets}
{\bbbf Proceedings of The American Mathematical Society} 118, No 2
%%%(June 1993)
, 577 - 586.
...
\bibitem{NSW} A. Nowik, M. Scheepers, T. Weiss.
...
\bibitem{NW2} A. Nowik, T. Weiss
\textit{The algebraic sum of a set of strong measure zero
and a perfectly meager set revisited}
{\bbbf East-West J. of Mathematics}, Vol. 2, No 2 (2000) pp. 191-194

\bibitem{P} J. Pawlikowski, \textit{All Sierpi\'nski sets are strongly
meager}, 1992, {\bbbf Arch. Mat. Logic} 35 (1996) 281 -- 285.

\bibitem{Si} W. Sierpi\'nski, \textit{Sur l'hypoth\`{e}se du continu 
